...
class AES(Crypto):
    ...
    def mixColumns(self, state):
        return [mult_8(state[i], 2)^mult_8(state[i+4],3)^state[i+8]^state[i+12] for i in range(4)] + \
               [state[i]^mult_8(state[i+4], 2) ^mult_8(state[i+8], 3)^state[i+12] for i in range(4)] + \
               [state[i]^state[i+4] ^mult_8(state[i+8], 2)^mult_8(state[i+12], 3) for i in range(4)] + \
               [mult_8(state[i],3 )^state[i+4]^state[i+8]^mult_8(state[i+12], 2) for i in range(4)]
    
    def inv_mixColumns(self, state):
        return [mult_8(state[i], 14)^mult_8(state[i+4],11)^mult_8(state[i+8], 13)^mult_8(state[i+12], 9) for i in range(4)] + \
